...
for violate_law in violate_laws:
    ...
    # 최대 비율 갱신
    max_percentage = max(max_percentage, sub_df['count'].max())
...
# 그래프 제목 및 축 이름 설정
plt.title('각 구의 법규 위반 비율 (조건)')
plt.xlabel('구')
plt.ylabel('비율 (%)')

# y축 단위를 10%로 설정하고, 최대값을 데이터의 최대값보다 높게 설정
plt.yticks(np.arange(0, math.ceil(max_percentage*10)/10 + 0.1, 0.1), ['{}%'.format(i*10) for i in range(math.ceil(max_percentage*10) + 1)])
